        ...
        case START_OTA:
            if(command.getPayloadSize() == PAYLOAD_SIZE_OFFSET + 1) {
                if(command.getPayload()[COMMAND_DATA] == 1 || command.getPayload()[COMMAND_DATA] == 2) {
                    ...
                    if(payload_data[COMMAND_RESPONSE] == COMMAND_ERROR) set_error(payload_data[COMMAND_DATA]);

                } else set_error(SLOT_OUT_OF_RANGE);
            } else set_error(PARAMETER_MISMATCH);
            break;
...
void SoftwareUpdateService::start_OTA(unsigned char slot_number) {
    ...
    if((state_flags & UPDATE_FLAG) == 0) {
        unsigned char state;
        if((error = fram_read_bytes((METADATA_SIZE + PAR_CRC_SIZE) * slot_number, &state, 1)) != NO_ERROR) return set_error(error);
        ...
        if(state == EMPTY) {
            ...
            state = PARTIAL;
            if((error = fram_write_bytes((METADATA_SIZE + PAR_CRC_SIZE) * slot_number, &state, 1)) != NO_ERROR) return set_error(error);
//            missed_blocks.arr = nullptr;
//            missed_blocks.arr_size = 0;
//            missed_blocks.num = 0;

        } else return set_error(SLOT_NOT_EMPTY);
    } else return set_error(UPDATE_ALREADY_STARTED);
}
...
void SoftwareUpdateService::receive_metadata(unsigned char* metadata) {
    ...
    if((state_flags & UPDATE_FLAG) > 0) {
        if((state_flags & METADATA_FLAG) == 0) {
            if((error = fram_write_bytes((METADATA_SIZE + PAR_CRC_SIZE) * update_slot + 1, metadata, METADATA_SIZE - 1)) != NO_ERROR) return set_error(error);
            num_update_blocks = metadata[NUM_BLOCKS_OFFSET - 1] | (metadata[NUM_BLOCKS_OFFSET] << 8);
            state_flags |= METADATA_FLAG;
        } else return set_error(METADATA_ALREADY_RECEIVED);
    } else return set_error(UPDATE_NOT_STARTED);
}
...
void SoftwareUpdateService::send_metadata(unsigned char slot_number) {
    ...
    if((error = fram_read_bytes((METADATA_SIZE + PAR_CRC_SIZE) * slot_number, &payload_data[COMMAND_DATA], METADATA_SIZE)) != NO_ERROR) return set_error(error);
}
        ...
        if((state_flags & METADATA_FLAG) > 0) {
            if((state_flags & PARTIAL_CRC_FLAG) == 0) {
                if(received_par_crcs < num_update_blocks * BLOCK_SIZE) {
                    if((error = fram_write_bytes((METADATA_SIZE + PAR_CRC_SIZE) * update_slot + METADATA_SIZE + received_par_crcs, crc_block, num_bytes)) != NO_ERROR) return set_error(error);

                    received_par_crcs += num_bytes;
                } else return set_error(PARAMETER_MISMATCH);
            } else return set_error(PARTIAL_ALREADY_RECEIVED);
        } else return set_error(METADATA_NOT_RECEIVED);
    } else return set_error(UPDATE_NOT_STARTED);
}
...
//    } else set_error(OFFSET_OUT_OF_RANGE);
//
//    return data;
    return nullptr;
}
                ...
                if(received_par_crcs < num_update_blocks * BLOCK_SIZE) {
                    if(block_offset < num_update_blocks) {
                        if(check_partial_crc(data_block, block_offset)) {
                            if((error = slot_write_bytes(update_slot, block_offset * BLOCK_SIZE, data_block, BLOCK_SIZE)) != NO_ERROR) return set_error(error);
                        } else {
...
//                            missed_blocks.num++;
                            return set_error(CRC_MISMATCH);
                        }
                    } else return set_error(OFFSET_OUT_OF_RANGE);
                } else return set_error(PARAMETER_MISMATCH);
            } else return set_error(PARTIAL_ALREADY_RECEIVED);
        } else return set_error(METADATA_NOT_RECEIVED);
    } else return set_error(UPDATE_NOT_STARTED);
...
void SoftwareUpdateService::check_md5(unsigned char slot_number) {
    ...
    uint16_t num_blocks;
    if((error = fram_read_bytes((METADATA_SIZE + PAR_CRC_SIZE) * slot_number + NUM_BLOCKS_OFFSET, (unsigned char*)&num_blocks, sizeof(uint16_t))) != NO_ERROR) return set_error(error);

    unsigned char meta_crc[CRC_SIZE];
    if((error = fram_read_bytes((METADATA_SIZE + PAR_CRC_SIZE) * slot_number + CRC_OFFSET, meta_crc, CRC_SIZE)) != NO_ERROR) return set_error(error);

    //TODO
    unsigned char* buffer = (unsigned char*)malloc(num_blocks * BLOCK_SIZE * sizeof(unsigned char));
    if(buffer == nullptr) return set_error(MEMORY_FULL);
...
void SoftwareUpdateService::stop_OTA() {
    ...
    if((state_flags & UPDATE_FLAG) > 0) {
        ...
        if(payload_data[COMMAND_RESPONSE] == COMMAND_ERROR) return set_error(payload_data[COMMAND_DATA]);
        if(payload_data[COMMAND_DATA]) {
           unsigned char temp = FULL;
           if((error = fram_write_bytes((METADATA_SIZE + PAR_CRC_SIZE) * update_slot, &temp, 1)) != NO_ERROR) return set_error(error);
    ...
    //           missed_blocks.num = 0;
           state_flags &= ~UPDATE_FLAG;
           update_slot = 0;
        } else return set_error(MD5_MISMATCH);
    } else return set_error(UPDATE_NOT_STARTED);
}
...
void SoftwareUpdateService::set_error(unsigned char error) {
